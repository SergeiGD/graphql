...
from ...db_gateways.workers_gateway import WorkersGateway
from ...db_gateways.groups_gateway import GroupsGateway
...
def resolve_add_group_to_worker(*_, worker_id: int, group_id: int, current_user):
    with get_session() as db:
        ...
        if worker is None:
            return return_not_found_error(Worker.REPR_MODEL_NAME)
        group: Group = GroupsGateway.get_by_id(group_id)
        if group is None:
